...
export interface OperationalWorkspaceAccessInput {
  ...
  readonly isolationScopeId: string;
...
export interface OperationalWorkspaceSessionStore {
  ...
  authenticateEmployee(employeeId: string): OperationalWorkspaceSession | null;
  logoutEmployee(): OperationalWorkspaceSession | null;
  disconnect(projectId: string, workspaceId: string): boolean;
  clear(): void;
}
...
export interface OperationalWorkspaceExecutionContext {
  readonly projectId: string;
  readonly solutionId: string;
  readonly solutionInstallationId: string;
  readonly isolationScopeId: string;
